...
import os
from tkcalendar import Calendar, DateEntry # Calendar, DateEntry no se usan aquí directamente, podrían eliminarse si no hay planes futuros.
...
class CasosTab(ttk.Frame):
    ...
    def load_cases_by_client(self, client_id):
        ...
        try:
            ...
            for case_data in cases: # Renombrado a case_data para evitar conflicto con el módulo 'case'
                ...
                num_exp = case_data.get('num_expediente', '')
    ...
    def on_client_changed(self, client_data):
        if client_data:
            ...
            self.clear_case_list()
            self.current_client_id = None # Asegurarse que current_client_id se limpia