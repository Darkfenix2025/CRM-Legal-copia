...
class AudienciasTab(ttk.Frame):
    ...
    def abrir_dialogo_audiencia(self, audiencia_id=None):
        ...
        caso_entry = ttk.Entry(caso_frame, textvariable=caso_id_var, width=25) # Ancho ajustado
        caso_entry.grid(row=0, column=0, sticky=tk.EW)
        ttk.Button(caso_frame, text="Buscar", command=lambda: self._seleccionar_caso_para_audiencia(caso_id_var, dialog)).grid(row=0, column=1, padx=(5, 0))
        row += 1
    ...
    def _seleccionar_caso_para_audiencia(self, caso_id_var, parent_dialog):
        ...
        try:
            ...
            elif caso_id_input: # Si no es dígito o está vacío pero no es None (cancelado)
                 messagebox.showwarning("Entrada inválida", "El ID del caso debe ser un número.", parent=parent_dialog)
